...
The reviewer runs from an external private working directory with `--safe-mode --toolsets , --max-turns 1 --query-file <private-prompt-path>`; under the pinned Hermes CLI the comma-only selector intentionally resolves to an empty toolset list. The private query file contains the manifest-bound diff prompt, and the host verifies the structured artifact before acceptance. This enforced boundary means `isolation remediation is pending` is not a current hold. Hermes writes its review artifact outside the public checkout, under the configured private report root, binding it to the same exact state as the manifest.
